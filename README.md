...
Определяет контракт для работы с коллекцией карточек. Помимо списка товаров, интерфейс предоставляет метод для поиска карточки по её идентификатору.

```
export interface IProductCatalog {
  items: IProductCard[];                           // Массив карточек товаров
  findCardById(cardId: string): IProductCard;        // Метод поиска карточки по id
};
```
...
- render()  
  Обеспечивает рендеринг элемента на странице.
...
Класс Modal, наследуемый от базового класса Component, отвечает за работу с модальными окнами. Он поддерживает несколько шаблонов модальных окон и использует EventEmitter для управления событиями.
...
### Методы
- Сеттер для description – обновляет информацию об описании товара.
...
### Свойства
- _submit – HTMLButtonElement, отвечающий за отправку формы и переход к следующему этапу (например, открытию другого модального окна).
- _errors – HTML-элемент, в котором выводятся сообщения об ошибках при неполном или неверном заполнении полей.

### Методы
- Сеттер errors – устанавливает и выводит текст ошибки, возникающей при неправильном или неполном вводе данных.
- render() – выполняет отрисовку формы с учётом всех ошибок и сообщений валидации.
